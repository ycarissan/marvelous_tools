...
class Geometry:
    def __init__(self, filename, orient = False):
        ...
        self.spherecenters = []
        self.pga = None
        self.xmin, self.ymin, self.zmin =  float("inf"),  float("inf"),  float("inf")
        self.xmax, self.ymax, self.zmax = -float("inf"), -float("inf"), -float("inf")
...
#         methane = Molecule(["C", "H", "H", "H", "H"], coords)
    ...
    def getAveragePlane(self,tol=0.01):
        dx = [self.xmax - self.xmin, 0, 0]
        dy = [0, self.ymax - self.ymin, 0]
        dz = [0, 0, self.zmax - self.zmin]
        if np.norm(dx)<tol:
            return [(self.xmax-self.xmin)/2, 0.0, 0.0], np.cross(dy, dz)
        if np.norm(dy)<tol:
            return [(self.ymax-self.ymin)/2, 0.0, 0.0], np.cross(dz, dx)
        if np.norm(dz)<tol:
            return [(self.zmax-self.zmin)/2, 0.0, 0.0], np.cross(dx, dy)
        return
